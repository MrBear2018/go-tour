...
type Model struct {
	...
	IsDel      uint8  `json:"is_del"` // 用 is_del 字段实现软删除
}
...
func updateTimeStampForCreateCallback(scope *gorm.Scope) {
	if !scope.HasError() {
		...
		// 调用 scope.FieldByName 方法，获取当前是否包含所需的字段
		if createTimeField, ok := scope.FieldByName("CreatedOn"); ok {
			// 判断 Field.IsBlank 的值，可以得知该字段的值是否为空
		...
		if modifyTimeField, ok := scope.FieldByName("ModifiedOn"); ok {
			...
			// 入参类型为 interface{}，内部也就是通过反射进行一系列操作赋值
...
/**
更新行为的回调
调用 scope.Get("gorm:update_column") 去获取当前设置了标识 gorm:update_column 的字段属性。
